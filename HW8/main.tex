...
.\end{align}
From these, it is clear that $B_{nm} = 0$ for all $m$ and consequently $n$.
Furthermore, it is similarly clear that $A_{nm} = 0$ if $m \ne 1$.
We therefore, only have to look at the $m=1$ terms in the second sum.
Actually, the problem simplifies further since the potential is orthogonal to all terms $n \ne 1$ as well, leaving us with the only nonzero coefficient
\begin{eqnarray}
